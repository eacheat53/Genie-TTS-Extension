...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "play-tts") {
    streamTTS(message.text, message.speed, message.character);
  }
});

async function streamTTS(textToRead, currentSpeed, characterName) {
  try {
    ...
    if (audioCtx.state === "suspended") {
      try {
        ...
        console.warn("audioCtx.resume() failed, but continuing:", e);
      }
    }
    ...
    const response = await fetch("http://127.0.0.1:8000/tts", {
      ...
      body: JSON.stringify({
        character_name: characterName || "feibi", // 动态获取面板设置的角色
        text: textToRead,
        text_language: "hybrid",
        split_sentence: true,
        speed: currentSpeed, // 使用用户实时选择的语速
      }),
    });
